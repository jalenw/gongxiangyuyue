...
- (void)savePhotoWithVideo:(NSURL *)videoPath completion:(void (^)(NSError *error))completion{
//    if (iOS9Later) { // 这里有坑... iOS8系统下这个方法保存图片会失败 原来是因为PHAssetResourceType是iOS9之后的...
//        NSData *data = [[NSData alloc] initWithContentsOfURL:videoPath];
//        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
//            PHAssetResourceCreationOptions *options = [[PHAssetResourceCreationOptions alloc] init];
//            options.shouldMoveFile = YES;
//            [[PHAssetCreationRequest creationRequestForAsset] addResourceWithType:PHAssetResourceTypeVideo data:data options:options];
//        } completionHandler:^(BOOL success, NSError * _Nullable error) {
//            dispatch_sync(dispatch_get_main_queue(), ^{
//                if (success && completion) {
//                    completion(nil);
...
//    } else {
        ...
        [self.assetLibrary writeVideoAtPathToSavedPhotosAlbum:videoPath completionBlock:^(NSURL *assetURL, NSError *error) {
            if (error) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
    ...
    NSInteger fileSize = [[NSFileManager defaultManager] attributesOfItemAtPath:videoAsset.URL.relativePath error:nil].fileSize;
    ...
    completion(videoAsset.URL.relativePath);
    return;
...
//    if ([presets containsObject:AVAssetExportPreset1280x720]){
//        session = [[AVAssetExportSession alloc]initWithAsset:videoAsset presetName:AVAssetExportPreset1280x720];
//    }else
        if ([presets containsObject:AVAssetExportPreset960x540]) {
        session = [[AVAssetExportSession alloc]initWithAsset:videoAsset presetName:AVAssetExportPreset960x540];
    }else if ([presets containsObject:AVAssetExportPreset640x480]) {
        session = [[AVAssetExportSession alloc]initWithAsset:videoAsset presetName:AVAssetExportPreset640x480];
