...
- (void)downloadContentWithIndex:(NSInteger)index
{
    ...
    UIImage *image = [[EMSDImageCache sharedImageCache] imageFromDiskCacheForKey:urlStr];
    if(!image)
    {
        UIImageView* progressView = [_progressViewArr objectAtIndex:index];
        progressView.hidden = NO;
        progressView.width = ScreenWidth * 0.01;
        [[EMSDWebImageManager sharedManager] downloadWithURL:url
                                                   options:EMSDWebImageLowPriority
                                                  progress:^(NSInteger receivedSize, NSInteger expectedSize)
         {
             CGFloat progress = (expectedSize > 0) ? (receivedSize / (float)expectedSize) : 0.01;
             progressView.width = progress * ScreenWidth;
         }
                                                 completed:^(UIImage *aImage, NSError *error, EMSDImageCacheType cacheType, BOOL finished)
         {
             if (finished && aImage)
             {
...
- (void)loadImageView:(int)i
{
    ...
    {
        bigImage = [[EMSDImageCache sharedImageCache] imageFromDiskCacheForKey:(NSString*)bigImageInfo];
    }
